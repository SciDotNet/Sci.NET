...
public static class ReshapeExtensions
{
    ...
    public static VirtualTensor<TNumber> Reshape<TNumber>(this ITensor<TNumber> tensor, Shape shape)
        where TNumber : unmanaged, INumber<TNumber>
    {
        return shape.ElementCount != tensor.ElementCount
            ? throw new ArgumentException("The number of elements in a reshape operation must not change.")
            : new VirtualTensor<TNumber>(tensor, shape);
    }

    /// <inheritdoc cref="Reshape{TNumber}(Sci.NET.Mathematics.Tensors.ITensor{TNumber},Sci.NET.Mathematics.Tensors.Shape)"/>>
    public static VirtualTensor<TNumber> Reshape<TNumber>(this ITensor<TNumber> tensor, params int[] shape)
        where TNumber : unmanaged, INumber<TNumber>
    {
